...
function Backup-RegistryKey {
    ...
    if ($exitCode -ne 0 -or -not (Test-Path -LiteralPath $file)) {
        $details = ($output | ForEach-Object { $_.ToString() }) -join [Environment]::NewLine
        throw "Registry backup failed for '$regPath' (exit code $exitCode). $details"
    }
...
function Get-UserPrinterReferences {
    ...
    foreach ($sid in (Get-LoadedUserSIDs)) {
        ...
        foreach ($unc in $uncLookup.Keys) {
            ...
            $connectionKeyName = ",,$($Matches[1]),$($Matches[2])"
            $connectionPath = Join-Path -Path $connectionRoot -ChildPath $connectionKeyName
...
function Remove-UserPrinterReference {
    ...
    if ($Reference.Type -eq 'RegistryKey') {
        if (Test-Path -LiteralPath $Reference.Path) {
            Remove-Item -LiteralPath $Reference.Path -Recurse -Force -ErrorAction Stop
        }
...
try {
    ...
    if ($RemoveFromDriverStore) {
        ...
        if (-not $removeCommand.Parameters.ContainsKey('RemoveFromDriverStore')) {
            throw 'This version of the PrintManagement module does not support -RemoveFromDriverStore.'
        }
